...
const Header = ({withAnimation, directions}) => {
    ...
    useEffect(() => {
        if (headerInnerRef && headerInnerRef.current) {
            if (activeHeader) {
                headerInnerRef.current.classList.add('backdrop-blur-[50px]');
            } else {
                headerInnerRef.current.classList.remove('backdrop-blur-[50px]');
            }
    ...
    return (
        <>
            ...
            <header ref={headerRef} className="duration-300 w-screen fixed top-0 left-0 z-10">
                ...
                <div ref={headerInnerRef} className="w-full flex justify-between items-center duration-200 absolute left-0 top-0 z-[11]
